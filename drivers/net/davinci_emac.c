...
static void davinci_eth_mdio_enable(void)
{
    ...
    debug_emac("davinci_eth_mdio_enable() - Ok\n");
}
...
static int davinci_eth_phy_detect(void)
{
    ...
    if (phy_act_state == 0) {
        debug_emac(" No active PHYs\n");
        return(0);
    }

    debug_emac("ALIVE = 0x%08x\n", phy_act_state);

    for (i = 0; i < 32; i++) {
        if (phy_act_state & (1 << i)) {
            if (phy_act_state & ~(1 << i)) {
                debug_emac("More than one PHY\n");
                active_phy_addr = 2;
                debug_emac("Active PHY addr (detected %d, set for debug %d)\n", i, 2);
                return(1);
            } else {
                debug_emac("Active PHY addr = %d\n", i);
                //active_phy_addr = i;
                active_phy_addr = 2;
                debug_emac("Active PHY addr (detected %d, set for debug %d)\n", i, 2);
                return(1);
...
#ifdef D2XT006_BOARD
#ifdef CPU_1
static int gen_init_phy1(int phy_addr)
{
    return 1;
}

static int gen_is_phy_connected1(int phy_addr)
{
    return 1;
}

static int gen_get_link_speed1(int phy_addr)
{
    return 1;
}

static int gen_auto_negotiate1(int phy_addr)
{
    return 1;
}
#endif
...
#ifdef CPU_1
        mac_control |= (EMAC_MACCONTROL_FULLDUPLEX_ENABLE | EMAC_MACCONTROL_MIIEN_ENABLE);
#else
...
static int davinci_eth_rcv_packet (struct eth_device *dev)
{
    ...
    if ((rx_curr_desc) && ((status & EMAC_CPPI_OWNERSHIP_BIT) == 0)) {
        if (status & EMAC_CPPI_RX_ERROR_FRAME) {
            ...
            debug_emac ("WARN: emac_rcv_pkt: Error in packet\n");
        } else {
        ...
        if (status & EMAC_CPPI_EOQ_BIT) {
            if (emac_rx_active_head) {
                ...
                debug_emac ("INFO:emac_rcv_packet: RX Queue not active\n");
            }
        }
...
int davinci_emac_initialize(void)
{
    ...
    if (!davinci_eth_phy_read(active_phy_addr, PHY_PHYIDR1, &tmp)) {
        active_phy_addr = 0xff;
        debug_emac("No active PHY detected!!!\n");
        return(0);
    }

    phy_id = (tmp << 16) & 0xffff0000;

    debug_emac("PHYID1: 0x%x\n", phy_id);
    ...
    case PHY_KSZ8051MNL_REV2:
        ...
        /* Additional PHY initialization */
        davinci_eth_phy_write(active_phy_addr, 0, (1 << 15));
        udelay(10000);
        //davinci_eth_phy_write(active_phy_addr, 0, (1 << 15));
        udelay(10000);
        davinci_eth_phy_write(active_phy_addr, 0x16, 1);
        udelay(10000);
        davinci_eth_phy_write(active_phy_addr, 0, (1 << 8) | (1 << 9) | (1 << 12) | (1 << 13));
        udelay(10000);
        break;
#endif
#if defined(PHY_KSZ8873MNL)
    case PHY_KSZ8873MNL:
        sprintf(phy.name, "KSZ8873MNL @ 0x%02x", active_phy_addr);
#ifdef CPU_1
        phy.init = gen_init_phy1;
        phy.is_phy_connected = gen_is_phy_connected1;
        phy.get_link_speed = gen_get_link_speed1;
        phy.auto_negotiate = gen_auto_negotiate1;
#else
        phy.init = gen_init_phy;
        phy.is_phy_connected = gen_is_phy_connected;
        phy.get_link_speed = gen_get_link_speed;
        phy.auto_negotiate = gen_auto_negotiate;
        udelay(20000);
        davinci_eth_phy_write(active_phy_addr, 0, 0x1300);
        udelay(20000);
#endif
        break;
#endif

...
static void pcie_config_ex(void)
{
   ...
   __raw_writel(0x00000002, 0x48180578);

   /* CM_DEFAULT_PCI_CLKCSTCTRL */
    __raw_writel(0x00000000, 0x48180510);
}
...
#if CONFIG_CMD_LED
void set_muxconf_leds(void)
{
    /* GP1[7:10] LED Muxconfig */
    __raw_writel(0xC0080, LED1_RED);
    __raw_writel(0xE0080, LED1_GREEN);
    __raw_writel(0xC0080, LED2_RED);
    __raw_writel(0xE0080, LED2_GREEN);

    /* MLBP_SIG_IO_CTRL */
    __raw_writel(0x3C, 0x48140E18);

    /* MLBP_DAT_IO_CTRL */
    __raw_writel(0x3C, 0x48140E1C);
}

void led_init(void)
{
    __raw_writel(0x8, 0x4804C010);          // GPIO_SYSCONFIG
    __raw_writel(0x2, 0x4804C130);          // GPIO_CTRL
    __raw_writel(~LED_PINS, 0x4804C134);    // GPIO_OE
    __raw_writel(LED_PINS, 0x4804C13C);     // GPIO_DATA_OUT
}

void led1_red_on(void)
{
    u32 data = __raw_readl(0x4804C13C);
    data &= ~(LED1_RED_PIN | LED1_GREEN_PIN);
    data |= LED1_RED_PIN;
    __raw_writel(data, 0x4804C13C);
}
...
{
    u32 data = __raw_readl(0x4804C13C);
    data &= ~(LED2_RED_PIN | LED2_GREEN_PIN);
    data |= LED2_RED_PIN;
    __raw_writel(data, 0x4804C13C);
}

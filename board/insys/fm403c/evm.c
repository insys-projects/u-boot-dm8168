...
void led1_green_on(void);
void led1_red_off(void);
void led1_green_off(void);
...
int board_init(void)
{
	...
	/* address of boot parameters */
        gd->bd->bi_boot_params = PHYS_DRAM_2 + 0x100;
...
{
	/* Fill up board info */
        gd->bd->bi_dram[0].start = PHYS_DRAM_2;
        gd->bd->bi_dram[0].size = PHYS_DRAM_2_SIZE;
 ...
 */
	...
	if(USE_EMIF1){
        ...
        walking_one_test(0xC0000000, 0xCfffffff);
#endif
}
...
void set_muxconf_regs(void)
{
	...
	__raw_writew((PTU | EN | M0), MMC_DAT3);
        ...
        /* GP0[31:28] LED Muxconfig */
        __raw_writew((PTU | EN | M1), LED1_RED);
        __raw_writew((PTU | EN | M1), LED1_GREEN);
        __raw_writew((PTU | EN | M2), LED2_RED);
        __raw_writew((PTU | EN | M2), LED2_GREEN);
...
/******************************************************************************
 * led_xxx(void) - Working with onboard LED
 *****************************************************************************/
void led_init(void)
{
    // init GP0[31:28] as output
    __raw_writel(0xF0000000, 0x4803213C);
    __raw_writel(0x0FFFFFFF, 0x48032134);

    // init GP1[26:25] as output
    __raw_writel(0x0, 0x4804C13C);
    __raw_writel(0x0FFFFFFF, 0x4804C134);
}

void led1_red_on(void)
{
    int val = __raw_readl(0x4803213C) & (~0x30000000);
    val |= 0x10000000;
    __raw_writel(val, 0x4803213C);
}
...
void led2_red_on(void)
{
    int val = __raw_readl(0x4803213C) & (~0xC0000000);
    val |= 0x40000000;
    __raw_writel(val, 0x4803213C);
}
...
void led1_red_off(void)
{
    int val = __raw_readl(0x4803213C);
    val &= ~0x10000000;
    __raw_writel(val, 0x4803213C);
}
